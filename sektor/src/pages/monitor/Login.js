...
const Login = () => {
    ...
    const formHandler = async (e) => {
        ...
        try {
            const resp = await axios.post(`${baseUrl}get_login.php`, formData);
            if(resp.data===0) {
                toast.dismiss()
                ...
                toast.dismiss()
                window.localStorage.setItem("monitor", sector)
                    ...
                    <div className="login_content">
                        ...
                        <div style={{marginTop: "32px"}} className="input-container">
                            ...
                            <select onChange={(e)=> setSector(e.target.value)} className="input-field" type="text" placeholder="Username" name="usrnm">
                                <option selected disabled>Секторни танланг</option>
                                <option value='1'>1-Сектор</option>
                                <option value='2'>2-Сектор</option>
                                <option value='3'>3-Сектор</option>
                                <option value='4'>4-Сектор</option>
                            </select>
                        </div>
