...
const Royxatlar = ({setInfo3,clearInfo,setOzgar,formInfo,info2,btn_iz,info1,tuman_id,date, handleOpen, setCurInfo1, setTumanId, setBtnIz, setInputs }) => {
      ...
      if(info2) {
        return (
            <tr className={info2.status === "edit" ? "tuman_edit": "null"}>
                <td><input disabled={disable ? "disabled": false} name="tav_muvof" onChange={handleInput} onKeyUp={(evt)=> {
                     var keycode = evt.charCode || evt.keyCode || evt.code;
                     console.log(evt.target.value)
                     if (keycode == 46) {
                       return false;
                     }
                }} className={info2.status === "edit" ? "input_edit": ""} type="number" min="0" max="100" value={info ? (info.tav_muvof ? info.tav_muvof: ""): ""}/></td>
                <td>{info && info.tav_muvof_iz ? <button value="tav_muvof_iz" onClick={handleIz1} disabled={disable ? "disabled": false} className={info2.status === "edit" ? "input_edit iz_btn": "iz_btn"}> <i style={{color: "blue"}} className="fas fa-check"></i></button> : <button value="t_muvof_iz" onClick={handleIz1} className={info2.status === "edit" ? "input_edit iz_btn hov": "iz_btn"}> <i className="fas fa-plus"></i></button>}</td>
                <td><input disabled={disable ? "disabled": false} name="um_ish_t_et" onChange={handleInput} className={info2.status === "edit" ? "input_edit": ""} type="number" min="0" max="100" value={info ? (info.um_ish_t_et ? info.um_ish_t_et: ""):""}/></td>
                ...
                <td style={{fontSize: "20px"}}>{info ? (info.um_ish_t_et ?   Math.floor((parseInt(info.tav_muvof)+parseInt(info.um_ish_t_et))/2) : null): null}</td>
                <td><input disabled={disable ? "disabled": false} name="b_sif" onChange={handleInput} className={info2.status === "edit" ? "input_edit ": ""} type="number" min="0" max="100" value={info ? (info.b_sif ? info.b_sif:""): ""}/></td>
                ...
                <td><input disabled={disable ? "disabled": false} name="um_ish_t_et1" onChange={handleInput} className={info2.status === "edit" ? "input_edit": ""} type="number" min="0" max="100" value={info ? (info.um_ish_t_et1 ? info.um_ish_t_et1: ""): ""}/></td>
                ...
                <td style={{fontSize: "20px"}}>{info ? (info.um_ish_t_et1 ?  Math.floor((parseInt(info.b_sif)+parseInt(info.um_ish_t_et1))/2) : null):null}</td>
                <td style={{fontSize: "20px"}}>{info ? (info.um_ish_t_et1 ? Math.floor(((parseInt(info.tav_muvof)+parseInt(info.um_ish_t_et))/2+(parseInt(info.b_sif)+parseInt(info.um_ish_t_et1))/2)/2) : ""):""}</td>
                ...
                <td><i onClick={deleteBaho} style={info2.status === "edit" ? {padding:"0 15px",fontSize: "18px", color:"#51686C", cursor: "pointer"}: {padding:"0 15px",fontSize: "18px", color:"#51686C"}} className="fas fa-trash"></i></td>  
            </tr>
        )
      }else {
        return null
