...
const Login = ({ setUser}) => {
    ...
    const formHandler = async (e) => {
        ...
        try {
            const resp = await axios.post(`${baseUrl}check_admin.php`, formData);
            if(resp.data===0) {
                console.log("sad")  
                toast.dismiss()
                ...
                toast.dismiss()
                window.localStorage.setItem("user1", sector)
                setUser(sector)
            }
        } catch (err) {
            // Handle Error Here
            ...
            <div style={{backgroundColor: "#E5E5E5", width: "100vw", height: "100vh", display: "flex", justifyContent: "center", alignItems: "center"}}>
                <div className="login_area">
                    <div className="login_content">
                        <h2 style={{marginBottom: "5px"}}><img style={{verticalAlign: "middle"}} src="Vector.png"></img><span style={{verticalAlign: "middle",fontFamily: "Roboto", fontSize: "26px"}} className="sektor_text">Админ Панел</span></h2>
